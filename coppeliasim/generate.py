...
# NOTE: do not change this; this is the path to all texture images:
textures_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'textures')

# NOTE: all letters of the alphabet:
        ...
        for x in range(tally['block'][atr]):
            ...
            if block_type == 'colours':
                ...
                for L in attributes['alphabets']:
                    handle, texture_id, _ = sim_interfacer.sim.createTexture(os.path.join(os.getcwd(), textures_dir, f'{L.lower()}.png'), 13)
...
if __name__ == '__main__':
    ...
    randomize_packing(
        path='../scenes/panda_packing_prototype.ttt',
        colours=colours,
    )